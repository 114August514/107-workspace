...
普通缺陷表示已有行为不符合预期，继续使用缺陷 Issue 跟踪，不写入
`docs/product/deferred.md`。尚未被明确接受的技术债也只保留在 Issue 中。只有决定在
当前代码中有意保留的实现妥协，才必须登记到
[`../product/deferred.md`](../product/deferred.md)，并在引入该妥协的 PR 中引用登记 ID。
