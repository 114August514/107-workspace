...
页面 `BrandMark.tsx` 与 `frontend/public/favicon.svg` 复用同一套 final path 和黑色静态填充；为让图形在 16 / 24 / 32 px
容器内占满接近 GitHub 常见产品 Mark 的可视面积，页面使用紧凑 `viewBox="20 20 101 101"`，不改变宽高比。
favicon 是同几何的专用静态光学版本，额外声明 `width="32" height="32"`，并使用同一紧凑 viewBox；不再使用旧 Mark
的 optical padding，也不保留运行时 Mark 切换或 fallback。原始 `107pig.svg` 保留在
