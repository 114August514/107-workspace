...
107 平台当前运行 Ubuntu 24.04.3，节点共享 `/public` 与 `/home`，使用 Environment Modules，普通用户无 sudo。`docs/references/platform/` 中的平台 PDF 是本决定的平台事实来源。Workspace 的真实 UID/GID、容器挂载、Slurm REST 与凭据集成仍属于 #7。
