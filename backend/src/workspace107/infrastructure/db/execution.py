...
from ...application.execution_context import ExecutionContextService
...
from ...domain.models import Activity, Artifact, Notification, Run, RunEvent, SharedResourceVersion
from ...domain.ports.execution import ExecutionContextPort, ExecutionStore, RunInputUnavailable
...
class SqlExecutionContext(ExecutionContextPort):
    """Short-transaction adapter around the shared execution-context service."""
    ...
    async def validate(self, run: Run, snapshot: RunSnapshot) -> ValidatedExecutionContext:
        async with self._factory() as session, session.begin():
            ...
            current_run = await repos.runs.get(run.id)
            if current_run is None:
                raise ValidationFailed(f"Run {run.id} 已不存在")
            current_snapshot = await repos.run_snapshots.get(current_run.snapshot_id)
            if current_snapshot is None:
                raise ValidationFailed(
                    f"Run {run.id} 的 exact Snapshot {current_run.snapshot_id} 已不存在"
                )
            if current_run.snapshot_id != snapshot.id or current_snapshot != snapshot:
                raise ValidationFailed("Worker 选取的 Run/Snapshot 已不再匹配持久事实")

            vault = DatabaseSecretVault(session)
            service = ExecutionContextService(
                repos,
                AccessGuard(repos),
                ScopedConfigResolver(repos.variables, vault),
            )
            return await service.validate(current_run, current_snapshot)
...
class SqlExecutionStore(ExecutionStore):
    ...
    async def resolve_inputs(self, run_id: str) -> tuple[RunWorkspaceInput, ...]:
        """Resolve the immutable Snapshot into materialization-only descriptors."""
        async with self._factory() as session, session.begin():
            repos = SqlRepositories(session)
            run = await repos.runs.get(run_id)
            if run is None:
                raise RunInputUnavailable(f"Run {run_id} 不存在")
            snapshot = await repos.run_snapshots.get(run.snapshot_id)
            if snapshot is None:
                raise RunInputUnavailable(f"Run {run_id} 缺少固定 Snapshot")

            resolved: list[RunWorkspaceInput] = []
            for binding in snapshot.input_bindings:
                if binding.source_type is InputSourceType.ARTIFACT:
                    artifact = await repos.artifacts.get(binding.source_id)
                    if artifact is None:
                        raise RunInputUnavailable(
                            f"输入 {binding.access_path} 引用的 Artifact 不存在"
                        )
                    if not artifact.is_available:
                        raise RunInputUnavailable(
                            f"输入 {binding.access_path} 引用的 Artifact 内容已被清理"
                        )
                    resolved.append(
                        ArtifactRunWorkspaceInput(
                ...
                if binding.source_type is InputSourceType.SHARED_RESOURCE_VERSION:
                    version = await repos.shared_resources.get_version_by_id(binding.source_id)
                    if version is None:
                        raise RunInputUnavailable(
                    ...
                    resolved.append(
                        SharedResourceRunWorkspaceInput(
                            ...
                            access_path=binding.access_path,
                            files=_resolve_shared_resource_files(
                                version,
                ...
                raise RunInputUnavailable(f"输入 {binding.access_path} 的来源类型不受 Worker 支持")
    ...
    async def retain_secret_redactions(self, run_id: str, values: list[str]) -> None:
        async with self._factory() as session, session.begin():
            await DatabaseSecretVault(session).retain_for_redaction(run_id, values)


def _resolve_shared_resource_files(
    version: SharedResourceVersion,
    source_subpath: str,
    *,
    access_path: str,
) -> tuple[RunWorkspaceInputFile, ...]:
    selected: list[RunWorkspaceInputFile] = []
    exact_file = next((item for item in version.files if item.path == source_subpath), None)
    if not source_subpath:
        candidates = ((item, item.path) for item in version.files)
    elif exact_file is not None:
        candidates = ((exact_file, exact_file.path.rsplit("/", 1)[-1]),)
    else:
        prefix = source_subpath + "/"
        candidates = (
            (item, item.path.removeprefix(prefix))
            for item in version.files
            if item.path.startswith(prefix)
        )

    for item, target_path in candidates:
        selected.append(
            RunWorkspaceInputFile(
    ...
    if not selected:
        detail = f"子路径 {source_subpath!r}" if source_subpath else "版本内容"
        raise RunInputUnavailable(f"输入 {access_path} 引用的 {detail} 不存在")
