...
class RunWorker:
    ...
    def __init__(
        ...
        *,
        ...
        execution_context: ExecutionContextPort,
    ...
    async def _process(self, pending: PendingExecution) -> None:
        ...
        try:
            ...
            secret_values = validated.secret_values
            if secret_values:
                await self._store.retain_secret_redactions(run.id, list(secret_values.values()))
            inputs = await self._store.resolve_inputs(run.id)
            identity = _workspace_identity(pending)
            paths = await self._workspace.prepare(identity, inputs=inputs)
        except (
            OSError,
            ProjectContentIdentityMismatch,
            ProjectContentMissing,
            ...
            detail = _safe_detail(exc, secret_values=secret_values.values())
            secret_values.clear()
            await self._store.record_submit_failed(run.id, detail)
            return
