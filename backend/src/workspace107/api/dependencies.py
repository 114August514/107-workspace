...
from workspace107.application.runs import ArtifactService, RunService
...
def get_cluster(request: Request) -> ClusterPort:
    cluster = getattr(request.app.state, "cluster", None)
    if cluster is None:
        raise RuntimeError("cluster adapter is not configured")
    return cast(ClusterPort, cluster)


ClusterDependency = Annotated[ClusterPort, Depends(get_cluster)]


def get_project_transport(request: Request) -> str:
    transport = getattr(request.app.state, "project_transport", None)
    if not isinstance(transport, str):
        raise RuntimeError("project transport is not configured")
    return transport


ProjectTransportDependency = Annotated[str, Depends(get_project_transport)]


def get_clock(request: Request) -> Callable[[], datetime]:
    clock = getattr(request.app.state, "clock", None)
    if not callable(clock):
        raise RuntimeError("application clock is not configured")
...
def get_artifact_service(
    ...
    return ArtifactService(uow_factory, storage)
