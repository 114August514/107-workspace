...
@dataclass(frozen=True, slots=True)
class RunWorkspaceInputFile:
    """Shared Resource blob copied to one relative path below an input binding."""
...
@dataclass(frozen=True, slots=True)
class SharedResourceRunWorkspaceInput:
    """Immutable Shared Resource version files fixed by the Run Snapshot."""

    version_id: str
    access_path: str
    files: tuple[RunWorkspaceInputFile, ...]


@dataclass(frozen=True, slots=True)
class ArtifactRunWorkspaceInput:
    """Canonical installed Artifact identity fixed by the Run Snapshot."""
...
RunWorkspaceInput = SharedResourceRunWorkspaceInput | ArtifactRunWorkspaceInput
...
class RunWorkspacePort(Protocol):
    async def prepare(
        ...
        *,
        ...
        """创建或恢复相同 identity 与输入证据的 workspace。"""
        ...
