...
def test_active_code_gr_references_exist_in_current_design() -> None:
...
def test_active_file_adr_references_exist() -> None:
    available = {
        match.group(1)
...
def test_active_file_docs_paths_exist() -> None:
    missing: list[str] = []
    for path in [*_active_files(), *_maintained_documents()]:
