...
down_revision: str | None = "fdb5011fe3b2"
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("runs", schema=None) as batch_op:
        batch_op.alter_column(
            "project_version_id", existing_type=sa.String(length=40), nullable=False
        )
        batch_op.alter_column(
            "project_version_label", existing_type=sa.String(length=32), nullable=False
        )
...
def _backfill_from_snapshots() -> None:
    ...
    for run_id, payload in rows:
        ...
        connection.execute(
            sa.text(
                "UPDATE runs SET project_version_id = :vid,"
                " project_version_label = :label WHERE id = :id"
            ),
